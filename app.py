...
class AppGUI(tk.Tk):
    ...
    # ============================== LECTURER MODULES ==============================
    def show_lecturer_assigned_classes(self):
        self.clear()
        self.build_sidebar('Lecturer')
        main = tk.Frame(self, padx=20, pady=20)
        main.pack(side="right", fill="both", expand=True)
        tk.Label(main, text="My Assigned Classes", font=("Arial", 16, "bold")).pack(anchor="w", pady=10)

        tree = ttk.Treeview(main, columns=("ccid", "subj", "sem"), show="headings")
        for c, t in zip(tree["columns"], ["Course Class ID", "Subject Name", "Semester"]):
            tree.heading(c, text=t)
        tree.pack(fill="both", expand=True)
        for c in self.srv.get_assigned_courses(self.user.user_id):
            tree.insert("", tk.END, values=(c['course_id'], c['subject'], c['semester']))

    def show_lecturer_profile(self):
        self.clear()
        self.build_sidebar('Lecturer')
        prof = self.srv.get_lecturer_profile(self.user.user_id)
        main = tk.Frame(self, padx=20, pady=20)
        main.pack(side="right", fill="both", expand=True)
        ...
        self.lp_name = tk.Entry(df, width=35)
        self.lp_name.insert(0, prof.full_name)
        self.lp_name.grid(row=0, column=1, pady=10)
        tk.Label(df, text="Email:").grid(row=1, column=0, sticky="w")
        self.lp_email = tk.Entry(df, width=35)
        self.lp_email.insert(0, prof.email)
        self.lp_email.grid(row=1, column=1, pady=10)
        tk.Label(df, text="Phone Number:").grid(row=2, column=0, sticky="w")
        self.lp_phone = tk.Entry(df, width=35)
        self.lp_phone.insert(0, prof.phone)
        self.lp_phone.grid(row=2, column=1, pady=10)
        tk.Button(df, text="Save Changes", bg="#16a34a", fg="white", command=self.upd_lecturer_profile).grid(row=3, column=1, sticky="e", pady=20)

    def upd_lecturer_profile(self):
        res = self.srv.update_lecturer_profile(self.user.user_id, self.lp_name.get(), self.lp_email.get(), self.lp_phone.get())
        if res == "OK":
            messagebox.showinfo("Success", "Profile updated successfully!")
        else:
            messagebox.showerror("Error", res)

    def show_lecturer_grades(self):
        self.clear()
        self.build_sidebar('Lecturer')
        main = tk.Frame(self, padx=20, pady=20)
        main.pack(side="right", fill="both", expand=True)
        ...
        sf.pack(fill="x", pady=5)
        tk.Label(sf, text="Search by Course Class ID:").pack(side="left")
        self.ent_search_gr = tk.Entry(sf, width=40)
        self.ent_search_gr.pack(side="left", padx=5)
        tk.Button(sf, text="Load Class List", command=self.load_lecturer_grades).pack(side="left", padx=5)

        self.tree_gr = ttk.Treeview(main, columns=("sid", "name", "att", "mid", "fin", "tot", "ccid"), show="headings")
        for c, t in zip(self.tree_gr["columns"], ["Student ID", "Full Name", "Attendance", "Midterm", "Final", "Total", "Course Class ID"]):
            self.tree_gr.heading(c, text=t)
        self.tree_gr.pack(fill="both", expand=True, pady=10)
        self.tree_gr.bind("<<TreeviewSelect>>", lambda e: self.on_select(self.tree_gr, [self.gr_sid, self.gr_name, self.gr_att, self.gr_mid, self.gr_fin]))

        df = tk.LabelFrame(main, text="Update Student Grades", padx=10, pady=10)
        df.pack(fill="x")
        self.gr_sid = tk.Entry(df)
        self.gr_name = tk.Entry(df)

        tk.Label(df, text="Attendance:").grid(row=0, column=0, sticky="w")
        self.gr_att = tk.Entry(df)
        self.gr_att.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(df, text="Midterm:").grid(row=0, column=2, sticky="w")
        self.gr_mid = tk.Entry(df)
        self.gr_mid.grid(row=0, column=3, padx=5, pady=5)

        tk.Label(df, text="Final:").grid(row=0, column=4, sticky="w")
        self.gr_fin = tk.Entry(df)
        self.gr_fin.grid(row=0, column=5, padx=5, pady=5)

        tk.Button(df, text="Save Changes", bg="#16a34a", fg="white", command=self.upd_lecturer_grade).grid(row=1, column=5, pady=10)

    def load_lecturer_grades(self):
        for r in self.tree_gr.get_children():
            self.tree_gr.delete(r)
        ccid = self.ent_search_gr.get()
        if not ccid:
            return
        for g in self.srv.get_grades_by_course(ccid):
            self.tree_gr.insert("", tk.END, values=(g['sid'], g['name'], g['att'], g['mid'], g['fin'], g['tot'], g['ccid']))

    def upd_lecturer_grade(self):
        ccid = self.ent_search_gr.get()
        if ccid and self.gr_sid.get():
            res = self.srv.update_grade(self.gr_sid.get(), ccid, self.gr_att.get(), self.gr_mid.get(), self.gr_fin.get())
            if res == "OK":
                self.load_lecturer_grades()
                messagebox.showinfo("Success", "Grades saved successfully!")
            else:
                messagebox.showerror("Error", res)
